...
- `manifest.webmanifest` 和 `sw.js`: PWA 安装信息与基础离线缓存；SW 对同源资源采用 network-first，安装预缓存绕过 HTTP 缓存，配合 `app.js` 在新 SW 接管后自动刷新。
